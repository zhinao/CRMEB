...
class UserStoreOrderServices extends BaseServices
{
    ...
    public function getUserSpreadCountList($uid, $orderBy = '', $keyword = '')
    {
        ...
        $field = "u.uid,u.nickname,u.rename,(SELECT nickname FROM eb_user p WHERE p.uid = u.spread_uid) AS p_nickname ,(SELECT `rename` FROM eb_user p2 WHERE p2.uid = u.spread_uid) AS p_rename, u.avatar,from_unixtime(u.add_time,'%Y/%m/%d') as time,u.spread_time,u.spread_count as childCount,p.orderCount,p.numberCount";
